...
public class AddLogContextPlugin extends PlayPlugin {
	...
	private void overrideMDC(Session session, Request request) {
		...
		if(username != null)
			return;
		else if(request.path.startsWith("/postdata")) {
			MDC.put("user", "__postdata__");
			return;
		} else if(user == null)
			return;
		else if("program".equals(user)) {
			String password = request.password;
			String full = user+":"+password;
			//The username program is used by everyone and their mom but their keys are ALL
			//unique and only work for certain tables.
			//override MDC as they are using basic auth
			MDC.put("user", full);
		} else
			MDC.put("user", user);
	}
